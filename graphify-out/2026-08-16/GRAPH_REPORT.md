...
## Corpus Check
- 66 files · ~140,887 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 762 nodes · 1550 edges · 71 communities (61 shown, 10 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 21 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4fe489a9`
...
7. `generateSchedule()` - 22 edges
8. `getEligibleShifts()` - 20 edges
...
Cohesion: 0.02
Nodes (58): RFC-5545, BASE_ELIGIBILITY, BLOCK_TARGETS, BLOCK_TYPE_MAP, BLOCK_TYPES_EM, BUTTON_SIZES, BUTTON_VARIANTS, CALENDAR_TIME_OF_DAY_LABEL (+50 more)

### Community 1 - "Synthetic Test Fixture Roster"
Cohesion: 0.10
Nodes (26): baselinePath(), captureFor(), captureOnce(), compareWithTolerance(), __dirname, errorCount(), loadBaseline(), makeBaselineSuite() (+18 more)
...
Cohesion: 0.25
Nodes (8): Known accepted gap: residents-never-see-schedule is UI-enforced only, blockLookup.js, netlify/functions/feedback-admin.js, feedback Supabase table, res_state Supabase table, sbFetch (hand-rolled PostgREST client), submitFeedback(), SUPABASE_ENABLED flag

### Community 5 - "Date Utilities"
Cohesion: 0.11
Nodes (32): applyTraumaClampAndDow(), AREA_NORMAL_IDS, CONF_AUTO_SWAP_12H_IDS, CONF_SUPPRESSED_NORMAL_IDS, DEFAULT_COVERAGE, DEFAULT_COVERAGE_MINMAX, DOW_COVERAGE_MAX_OVERRIDE, getCoverageFor() (+24 more)

### Community 6 - "Schedule Generator Core"
Cohesion: 0.07
Nodes (76): twelveHourStateFor(), addDays(), formatAY(), getAcademicYearFor(), getBlockDates(), getBlockWeekends(), parseDate(), qgendaDate() (+68 more)
...
Cohesion: 0.10
Nodes (21): getAcademicYear(), AddResidentModal(), AvailabilityRangesEditor(), AY_CONF_DATE_FIELDS, BlockProgressBar(), DashboardTab(), DragConfirmModal(), EMResidentsTab() (+13 more)
...
Cohesion: 0.29
Nodes (7): AutosaveIndicator pill, Dashboard/Home merge (Home tab removed), getCoverageFor(), normalizeCoverageEntry(), reconcileTabOrder(), SidebarNav component, Treat all backup/cloud/hand-edited localStorage as untrusted shape
...
Cohesion: 0.38
Nodes (7): Cloud-op-succeeds-before-local-wipe/write discipline (avoid stale mount overlay reverting), Demo Sandbox feature, physKey() / demoPhysKey(), RES_STATE_DEMO_ROW_ID, SettingsTab.clearAll(), SettingsTab.importData(), syncSuspended flag

### Community 67 - "em-scheduler (sibling app, same author/domain)"
Cohesion: 0.33
Nodes (6): BlockCalendarSection (Dashboard year-timeline), computeCoverageByDate(), Dark mode (index.css override sheet, not Tailwind dark: variants), em-scheduler (sibling app, same author/domain), Modal / SectionCard / CollapsibleCard / SubTabs primitives, StatCard component

### Community 68 - "dbReady gate"
Cohesion: 0.33
Nodes (6): dbReady gate, Physical-key isolation, not mode-check-in-shared-code (discipline enforced per call site), demoCheckGenRef (probe generation counter), Last-write-wins / full-document-overwrite conflict handling (accepted tradeoff), openDemoModal(), sbLoadState()
...
Cohesion: 0.50
Nodes (5): enterDemoFresh(), LS_BACKUP_KEYS, res_dark_mode (device-local, excluded from backup), syncBindings map, useLocalStorage hook
